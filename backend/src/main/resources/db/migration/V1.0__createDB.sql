...
create table copies (id bigint not null, amount integer not null check (amount>=1), item bigint not null, uzer varchar(255) not null, primary key (id));
...
alter table copies add constraint UK9ohbgfo1cb8l8805yxlsbvaqb unique (uzer, item);
...
alter table copies add constraint FKbrl2ngwykmxtlyex49nrfrxnh foreign key (item) references items;
alter table copies add constraint FK8f4rstxmlxhbxm9qqkd431oi8 foreign key (uzer) references users;
